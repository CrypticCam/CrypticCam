...
class _ProfileTabState extends State<ProfileTab> {
  ...
  // --- KULLANICI VERİSİ VE DOĞUM GÜNÜ ---
  ...
  Future<void> _checkBirthdayGreeting(Map<String, dynamic> userData) async {
    ...
    if (today.day == birthday.day && today.month == birthday.month) {
      ...
      if (lastGreetingYear != today.year.toString()) {
        final chatRef = FirebaseFirestore.instance.collection('chats').doc("crypticcam_$_currentUsername");
        await chatRef.set({
        ...
        await FirebaseFirestore.instance.collection('users').doc(_currentUsername!).update({
          'lastGreetingYear': today.year.toString()
        });
      }
    }
  }

  // ---  BASE64 FOTOĞRAF YÜKLEME ---
  ...
  // --- TAKİPÇİ ÇIKARMA VE TAKİBİ BIRAKMA (BATCH) ---
      ...
      children: [
        Stack(
          children: [
            ...
            if (_isUploading) const Positioned.fill(child: CircularProgressIndicator(color: Color(0xFFADCFD0), strokeWidth: 2)),
            Positioned(bottom: 0, right: 0, child: GestureDetector(onTap: _pickAndUploadImage, child:
            const CircleAvatar(radius: 18, backgroundColor: Color(0xFFADCFD0), child: Icon(Icons.camera_alt, size: 18, color: Colors.white)))),
          ],
        ),
        const SizedBox(height: 15),
        Text("${data['ad'] ?? ''} ${data['soyad'] ?? ''}", style:
        TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)),
  ...
  // --- AKILLI VE FOTOĞRAFLI ARAMA SONUÇLARI ---
  Widget _buildSearchResults(bool isDark) {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final userDoc = _filteredUsers[index]; // Mevcut doküman
        final userData = userDoc.data() as Map<String, dynamic>; // Kullanıcı verisi
        final tUser = userDoc.id; // Kullanıcı adı

        // --- BASE64 PROFİL FOTOĞRAFI MANTIĞI ---
        final String? searchUserPic = userData['profilePic'];
        bool hasSearchImg = searchUserPic != null && searchUserPic.trim().isNotEmpty;

        return StreamBuilder<DocumentSnapshot>(
          // 1. KONTROL: Zaten takip listemde mi?
          ...
          builder: (context, followSnap) {
            ...
            // 2. KONTROL: Gönderilmiş bekleyen bir istek var mı?
            return StreamBuilder<DocumentSnapshot>(
              stream: FirebaseFirestore.instance
                  .collection('users')
                  .doc(tUser)
              ...
              builder: (context, requestSnap) {
                ...
                Color buttonColor = const Color(0xFFFBC8B6); // Şeftali
                bool isDisable = false;

                if (isAlreadyFollowing) {
                  buttonText = "Takip"; //
                  buttonColor = isDark ? Colors.white12 : (Colors.grey[300] ?? Colors.grey);
                  isDisable = true;
                } else if (isRequestSent) {
                  buttonText = "İstek Gönderildi"; //
                  buttonColor = isDark ? Colors.white10 : (Colors.grey[200] ?? Colors.grey);
                  isDisable = true;
                }

                return ListTile(
                  contentPadding: const EdgeInsets.symmetric(vertical: 8),
                  // --- YENİ: ARAMA SONUCUNDA PROFİL FOTOĞRAFI ---
                  leading: CircleAvatar(
                    radius: 24,
                    backgroundColor: isDark ? const Color(0xFF1A1A1A) : const Color(0xFFADCFD0),
                    backgroundImage: hasSearchImg
                        ? MemoryImage(base64Decode(searchUserPic!.trim())) //
                        : null,
                    child: !hasSearchImg
                        ? Icon(Icons.person, color: isDark ? Colors.white24 : Colors.white)
                        : null,
                  ),
                  title: Text("@$tUser",
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, idx) {
                      ...
                      return ListTile(
                        ...
                        // --- Çarpı butonu: Takipçi çıkar veya Takibi bırak ---
                        trailing: IconButton(
  ...
  Widget _buildSettingsButton() {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFADCFD0).withOpacity(0.1), foregroundColor:
      const Color(0xFFADCFD0), elevation: 0, side: const BorderSide(color: Color(0xFFADCFD0), width: 0.5),
  ...
  Widget _buildStatItem(String count, String label, IconData icon, String col, bool isDark) {
    return GestureDetector(
      ...
      child: Column(children: [Icon(icon, color: const Color(0xFFADCFD0), size: 20),
        const SizedBox(height: 4), Text(count, style:
        TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: isDark ? Colors.white : Colors.black87)),
        Text(label, style: const TextStyle(color: Colors.grey, fontSize: 11))]),
    );
  }

  Widget _buildQRCodeSection(bool isDark) {
    return Column(children: [Text("Hızlı Erişim QR", style:
    TextStyle(color: isDark ? Colors.white38 : Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
      const SizedBox(height: 15), Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: isDark ?
      const Color(0xFF1A1A1A) : Colors.white, borderRadius: BorderRadius.circular(30), border: Border.all(color: isDark ?
      const Color(0xFFADCFD0).withOpacity(0.3) : Colors.transparent)),
          child: QrImageView(data: _currentUsername!, version: QrVersions.auto, size: 150.0, eyeStyle:
          const QrEyeStyle(eyeShape: QrEyeShape.circle, color: Color(0xFFADCFD0)), dataModuleStyle:
          QrDataModuleStyle(dataModuleShape: QrDataModuleShape.circle, color: isDark ? Colors.white :
          const Color(0xFFFBC8B6))))]);
  }

  PreferredSizeWidget _buildAppBar(bool isDark) {
    return AppBar(backgroundColor: Colors.transparent, elevation: 0, leading:
    StreamBuilder<QuerySnapshot>(stream:
    FirebaseFirestore.instance.collection('users').doc(_currentUsername!).collection('requests').snapshots(),
        builder: (context, snapshot) { int count = snapshot.hasData ? snapshot.data!.docs.length : 0;
      return IconButton(icon: Badge(label: count > 0 ? Text(count.toString()) : null, isLabelVisible: count > 0,
          child: const Icon(Icons.notifications_none_rounded, color: Color(0xFFADCFD0), size: 28)),
          onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) =>
              NotificationsScreen(currentUsername: _currentUsername!)))); }),
        actions: [IconButton(icon: const Icon(Icons.qr_code_scanner_rounded, color: Color(0xFFADCFD0), size: 28), onPressed: _openQRScanner),
          const SizedBox(width: 15)]);
  }

  Widget _buildLogoutButton(bool isDark) {
    return SizedBox(width: double.infinity, child: TextButton.icon(onPressed: () async {
      await FirebaseAuth.instance.signOut();
      if (mounted) Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (context) => const LoginScreen()), (r) => false); },
        icon: const Icon(Icons.logout_rounded, size: 18), label: const Text("Güvenli Çıkış Yap"),
        style: TextButton.styleFrom(foregroundColor: Colors.redAccent.withOpacity(0.8),
            padding: const EdgeInsets.symmetric(vertical: 15))));
  }

  void _searchUser(String q) async {
    if (q.isEmpty) { setState(() => _isSearching = false); return; } setState(() => _isSearching = true);
    final results = await FirebaseFirestore.instance.collection('users').where(FieldPath.documentId,
        isGreaterThanOrEqualTo: q.toLowerCase()).where(FieldPath.documentId, isLessThanOrEqualTo: '${q.toLowerCase()}\uf8ff').get();
  ...
  Future<void> _sendFollowRequest(String t) async {
    ...
    try { await FirebaseFirestore.instance.collection('users').doc(t).collection('requests').doc(_currentUsername).set({'from': _currentUsername, 'timestamp': FieldValue.serverTimestamp(), 'status': 'pending'}); _msg("İstek gönderildi."); }
    catch (e) { _msg("Hata!"); } }
...
class QRScannerScreen extends StatelessWidget {
  const QRScannerScreen({super.key}); @override Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: const Text("QR Tara")),
        body: MobileScanner(onDetect: (capture) { final b = capture.barcodes;
